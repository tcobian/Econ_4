...
Regression fit
Sector A: Data looked relatively linear and a linear model described 0.904 of the variance of the data. 

Sector B: Data looked relatively linear and a linear model described 0.5355 of the variance of the data therefore the model did not fit the data as well as sector A.

Sector C: Data looked relatively linear and a linear model described 0.8663 of the variance of the data therefore the model did not fit the data as well as sector A.

Sectocr D: Data looked relatively linear and a linear model described 0.5108 of the variance of the data therefore the model did not fit the data as well as sector A.
...
# sector A: current carbon emissions are 180, cut down by 1/2 = 90
benefit_A <- integrate(A, 0, 90)

# sector B: current carbon emissions are 200, cut down by 1/2 = 100
benefit_B <- integrate(B_line, 0, 100)

# sector C: current carbon emissions are 220, cut down by 1/2 = 110
benefit_C <- integrate(C_line, 0, 110)
...
cost_A_tax <- integrate(A_2, 0, A_MC(opt_tax))

cost_B_tax <- integrate(B_2, 0, B_MC(opt_tax))

cost_C_tax <- integrate(C_2, 0, C_MC(opt_tax))

total_cost_tax <- cost_A_tax$value + cost_B_tax$value + cost_C_tax$value

tax_rev<- 300*opt_tax


benefit_A_tax <- integrate(A, 0, A_MC(opt_tax))

benefit_B_tax <- integrate(B_line, 0, B_MC(opt_tax))

benefit_C_tax <- integrate(C_line, 0, C_MC(opt_tax))
...
- Cost to sector A: \$`r round(cost_A_tax$value, 2)`
- Cost to sector B: \$`r round(cost_B_tax$value, 2)`
- Cost to sector C: \$`r round(cost_C_tax$value, 2)`
...
benefit_A_cap <- integrate(A, 0, A_MC(price))

benefit_B_cap <- integrate(B_line, 0, B_MC(price))

benefit_C_cap <- integrate(C_line, 0, C_MC(price))

total_benefit_cap<- benefit_A_cap$value + benefit_B_cap$value + benefit_C_cap$value

net_benefot_cap<- total_benefit_cap - total_cost_cap
...
- Cost to sector A: \$`r round(cost_A_cap$value, 2)` 
- Cost to sector B: \$`r round(cost_B_cap$value, 2)` 
- Cost to sector C: \$`r round(cost_C_cap$value, 2)`
- Net Benefit: \$`r round(net_benefot_cap, 2)`
...
benefit_A_4 <- integrate(A, 0, A_MC(new_price))

benefit_B_4 <- integrate(B_line, 0, B_MC(new_price))

benefit_C_4 <- integrate(C_line, 0, C_MC(new_price))

benefit_D_4 <- integrate(D_line, 0, D_MC(new_price))
...
net_benefit_y<- benefit_D_4$value - cost_D_4$value
...
- Cost to sector A: \$`r round(cost_A_4$value, 2)` 
- Cost to sector B: \$`r round(cost_B_4$value, 2)` 
- Cost to sector C: \$`r round(cost_C_4$value, 2)` 
- Net benefit country X: \$`r round(net_benefit_x, 2)`
- Net benefit country y: \$`r round(net_benefit_y, 2)`
- Country Y (\$`r round(cost_D_4$value, 2)`) would have an incentive to enter into the market becuse it is cheaper for them to abate than country X (\$`r round(total_cost_4, 2)`)
